...
function generateFriendlyPath() {
    const adjectives = [
        'free', 'instant', 'professional', 'premium', 'advanced', 'smart', 'quick', 'credit', 'optimized'
    ];
    const nouns = [
        'rental', 'onboarding','application', 'profile', 'skills', 'checker', 'review'
    ];
    const actions = [
        'free', 'instant', 'professional', 'premium', 'advanced', 'smart', 'quick', 'credit', 'optimized'
    ];
...
app.post('/api/create-link', requireAuth, (req, res) => {
    try {
        ...
        if (customPath) {
            ...
            switch (urlType) {
                case 'promo':
                    promoCode = 'rental' + Math.floor(Math.random() * 9000 + 1000);
                    break;
                case 'tool':
                    friendlyPath = 'tools/renta';
                    break;
                case 'company':
