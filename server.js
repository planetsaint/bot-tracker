  ...
  try {
    ...
    if (promoCode) {
      ...
      if (friendlyPath.includes("/")) {
        ...
        shortUrl =
          protocol + "://" + host + "/go/" + friendlyPath.split("/").pop();
      } else {
...
function handleTracking(req, res, trackingId) {
  ...
  if (trackingData.has(trackingId)) {
    ...
    console.log(
      "Bot interaction: " + clientInfo.ip + " - " + clientInfo.userAgent
    );
  } else {
...
app.get("/promo/:promoCode", (req, res) => {
  ...
  const trackingId =
    findTrackingIdByPromo(promoCode) || "promo-" + promoCode + "-" + Date.now();
  handleTracking(req, res, trackingId);
});
...
app.get("/api/dashboard", requireAuth, (req, res) => {
  ...
  res.json({
    ...
    botPercentage:
      totalClicks > 0 ? Math.round((totalBots / totalClicks) * 100) : 0,
...
app.get("/api/export/csv", requireAuth, (req, res) => {
  const csv = [
    "Timestamp,IP,UserAgent,IsBot,BotScore,Campaign,Description,Referrer",
  ];
  ...
  res.setHeader(
    "Content-Disposition",
    'attachment; filename="visitor-data.csv"'
  );
  res.send(csv.join("\n"));
});
...
app.listen(PORT, () => {
  ...
  console.log(
    "🔐 Admin token: " + (process.env.ADMIN_TOKEN || "admin-secret-token")
  );
